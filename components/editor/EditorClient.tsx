...
/**
 * Book/article context passed by the nested route (`/books/[bookId]/[articleId]/edit`)
 * so the editor can render a breadcrumb + author/book-metadata header, mirroring the
 * reader's `/books/...` context header. Null when the editor is reached via the legacy
 * `/documents/[id]/edit` route.
 */
export interface EditorBookContext {
    book: {
        id: string;
        title: string;
        author?: string | null;
...
export default function EditorClient({
    ...
    articleId: string;
    bookContext?: EditorBookContext | null;
}) {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const assnRes = await fetch(`/api/documents/${articleId}/assignments`);
  ...
  const refreshActivity = useCallback(async () => {
    try {
      const res = await fetch(`/api/documents/${articleId}/segment-activity`);
      if (!res.ok) return;
  ...
  const loadData = useCallback(async () => {
    ...
    const [art, segs] = await Promise.all([
      pb.collection('articles').getOne<{ id: string; title: string }>(articleId, { fields: 'id,title' }).catch(() => null),
      fetchAllSegments<Segment>(pb, articleId, targetLang),
    ]);
    if (art) setArticle(art);
    setSegments(segs);
    setLoading(false);
    void refreshActivity();
  }, [articleId, refreshActivity, targetLang, pb]);
  ...
  const handleBatchAdvance = async (toStatus: SegmentStatus) => {
    ...
    setBatchResult(null);
    try {
      const res = await fetch(`/api/documents/${articleId}/batch-advance`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  // Reader link destination: preserve the current route family (book vs legacy).
  const readerHref = bookContext
    ? `/books/${bookContext.book.id}/${articleId}/1`
    : `/documents/${articleId}/read`;
